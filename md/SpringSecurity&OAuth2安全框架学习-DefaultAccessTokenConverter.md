DefaultAccessTokenConverter先用接口org.springframework.security.oauth2.provider.token.UserAuthenticationConverter实现类
进行token信息转换，再进行后续操作，可以自己定义UserAuthenticationConverter进行拓展操作，如获取权限信息之中的租户信息并保存至本地线程之中，
后续相关操作从本地线程之中获取租户信息
```text
```
```java
